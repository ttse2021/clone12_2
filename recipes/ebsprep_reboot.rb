...
reboot_completed="#{node[:clone12_2][:ebsprep][:workingdir]}/t.rebooted_#{node[:hostname]}"
log     "FILE IS: #{node[:clone12_2][:ebsprep][:workingdir]}/t.rebooted_#{node[:hostname]}"
...
unless File.file?( reboot_completed )
  ...
  execute "kernel_rebooting_#{node[:hostname]}" do
    ...
    command "touch #{reboot_completed} && shutdown -Fr now && sleep 10"
